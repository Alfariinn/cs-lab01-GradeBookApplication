...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            if (parts[2] == "standard")
            {
                BaseGradeBook standardGradeBook = new StandardGradeBook(name,bool.Parse(isWeighted));
            ...
            {
                BaseGradeBook rankedGradeBook = new RankedGradeBook(name, bool.Parse(isWeighted));
                Console.WriteLine("Created ranked gradebook {0}.", name);
                GradeBookUserInterface.CommandLoop(rankedGradeBook);

            }
            else Console.WriteLine($"{parts[2]} is not a supported type of gradebook, please try again");
